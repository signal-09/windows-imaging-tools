...
$logFile = "$ENV:SystemDrive\image-generation.log"
...
function Delete-RecoveryPartition {
    ...
    $partition = Get-Partition | Where-Object { $_.GptType -eq '{DE94BBA4-06D1-4D40-A16A-BFD50179D6AC}' }

    if ($partition) {
        reagentc /disable
        Write-Log "WinRE" "Windows Recovery partition disabled"

        Remove-Partition -DiskNumber $diskNumber -PartitionNumber $partition.PartitionNumber -Confirm:$false
        Write-Log "WinRE" "Windows Recovery partition deleted"
        ...
        Write-Log "WinRE" "Disk $ENV:SystemDrive extended"
    }
...
function Install-QemuGuestAgent {
    ...
    $qemuGaMsiPath = "E:\guest-agent\qemu-ga-x86_64.msi"
    $qemuGaMsiLog = "$ENV:SystemDrive\qemu-ga.log"
...
try {
    ...
    if ($deleteRecoveryPartition) {
        Delete-RecoveryPartition
    }
